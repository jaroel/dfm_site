...
pub(crate) fn Controls(
  ...
  let (title, _) = create_signal(cx, title);
  let (label, _) = create_signal(cx, label);
  ...
  async fn has_loaded() -> bool {
    true
  }
  let client_has_loaded = create_local_resource(cx, move || (), |_| has_loaded());
        ...
        view! { cx,
          <div class="flex items-center">
            ...
            <audio
              ...
              preload="none"
              src=local_src
              class="mr-4 rounded-full border my-1 bg-gray-100 text-gray-800 border-gray-800"
            ></audio>
          </div>
        }
    }>
      <Show
        when=move || { client_has_loaded.read(cx).unwrap_or(false) }
