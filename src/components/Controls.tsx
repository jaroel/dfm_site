...
import { PLAYER_CTX } from "./Player";

export default component$(
	(props: { title: string; label: string; src: string }) => {
		const player_state = useContext(PLAYER_CTX);
		const control_state = useComputed$(() => {
			return player_state.src === props.src ? player_state.state : "stopped";
		});
		...
		return (
			<button
				...
				title={props.title}
				class={`my-1 mr-4 flex items-center rounded-full border px-4 py-2 ${
					{
